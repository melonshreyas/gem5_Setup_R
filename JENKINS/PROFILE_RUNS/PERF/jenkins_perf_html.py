#!/usr/bin/env python3
"""HTML reporting helpers for the gem5 PERF profile workflow."""
...
_REPORT_CSS_FILENAME = "perf_report.css"
_REPORT_CSS = """:root {
  color-scheme: light;
  --bg: #f6f8fb;
  --panel: #ffffff;
...
def render_perf_results_html(page_title: str, summary_fields: Dict[str, Any], rows: List[Dict[str, Any]]) -> str:
...
def generate_perf_results_json(output_dir: Path, logger: Any) -> None:
  """Generate the JSON smoke report for the latest run directory.
  It writes the structured run summary and per-case rows to RESULTS/perf_results.json.
  """
  ...
  report_path = output_dir / "RESULTS" / "perf_results.json"
  report_path.write_text(
    json.dumps(_build_report_payload("PERF Results", summary_fields, rows), indent=2),
    encoding="utf-8",
  )
  logger.warning(f"Wrote PERF JSON report: {report_path}")


def generate_perf_results_html(output_dir: Path, logger: Any) -> None:
    ...
    html_text = render_perf_results_html("PERF Results", summary_fields, rows)
    report_path = output_dir / "RESULTS" / "perf_results.html"
    (output_dir / "RESULTS" / _REPORT_CSS_FILENAME).write_text(_REPORT_CSS, encoding="utf-8")
    report_path.write_text(html_text, encoding="utf-8")
    logger.warning(f"Wrote PERF HTML report: {report_path}")


def generate_jenkins_history_perf_results_html(history_dir: Path, logger: Any, limit: int = 30) -> None:
    """Generate the HTML history report from recent smoke runs.
    It writes the latest build snapshots into jenkins_history_perf_results.html.
    """
    ...
    html_text = render_perf_results_html("Jenkins History PERF Results", summary_fields, selected_rows)
    report_path = history_dir / "jenkins_history_perf_results.html"
...
def generate_jenkins_history_perf_results_json(history_dir: Path, logger: Any, limit: int = 30) -> None:
    ...
    report_path = history_dir / "jenkins_history_perf_results.json"
    report_path.write_text(
        json.dumps(_build_report_payload("Jenkins History PERF Results", summary_fields, selected_rows), indent=2),
        encoding="utf-8",
    )
